...
class SimController:
    def __init__(self, config: SundayA1Config):
        ...
        self.controller_server = ControllerWebServer()
    ...
    def _setup_mujoco(self):
        ...
        self.mj_data.qpos[7:] = self.config.default_qpos
    ...
    def rl_controller(self):
        ...
        target_qpos = self.smoothed_target.apply(
            self.default_qpos + action * self.config.policy_config.action_scale
        )
        self.mj_data.ctrl[:] = np.clip(
            self.motor_pd_controller.update(
                target_qpos - self.qpos,
                derivative=self.qvel
            ),
            self.mj_model.actuator_ctrlrange[:, 0],
            self.mj_model.actuator_ctrlrange[:, 1]
    ...
    def run(self):
        ...
        with mujoco.viewer.launch_passive(self.mj_model, self.mj_data) as viewer:
            while viewer.is_running():
                self.qpos[:] = self.mj_data.qpos[7:]
                self.qvel[:] = self.mj_data.qvel[6:]
